...
int main()
{
    ...
    for (int i = 0; i < vector_rooms.size()/4; i++) {
        if (grid[(vector_rooms.at((i*4)+0))-1][(vector_rooms.at((i*4)+1)-1)] == ' ')
            grid[(vector_rooms.at((i*4)+0))-1][(vector_rooms.at((i*4)+1)-1)] = '-';
        if (grid[vector_rooms.at((i*4)+2)][vector_rooms.at((i*4)+3)] == ' ')
            grid[vector_rooms.at((i*4)+2)][vector_rooms.at((i*4)+3)] = '-';
        if (grid[(vector_rooms.at((i*4)+0))-1][vector_rooms.at((i*4)+3)] == ' ')
            grid[(vector_rooms.at((i*4)+0))-1][vector_rooms.at((i*4)+3)] = '-';
        if (grid[vector_rooms.at((i*4)+2)][(vector_rooms.at((i*4)+1))-1] == ' ')
            grid[vector_rooms.at((i*4)+2)][(vector_rooms.at((i*4)+1))-1] = '-';
    }
    int* tracer;
    tracer = new int[2];
    tracer[0] = 0;
            ...
            if (lastpos[1] != newpos[1]) {
                if (lastpos[1] < newpos[1]) {
                    for (int i = lastpos[1]; i != newpos[1]+1; i++) {
                        ...
                        grid[lastpos[0]][lastpos[1]] = '#';
                        if (lastpos[1] == newpos[1]) {
                            if (lastpos[0] < WHEIGHT)
                                grid[lastpos[0]+1][lastpos[1]] = '#';
                            if (lastpos[0] > 0)
                                grid[lastpos[0]-1][lastpos[1]] = '#';
                            if (lastpos[1]>0)
                                grid[lastpos[0]][lastpos[1]-1] = '#';
                            if (lastpos[1] < WWIDTH)
                                grid[lastpos[0]][lastpos[1]+1] = '#';
                            break;
                        }
